...
/* ---- BOOKING FORM SUBMIT — EmailJS only, no mailto ---- */
function submitBooking() {
  ...
  emailjs.send(
    ...
    // Mark date as booked in localStorage
    ...
    showBookingError('Something went wrong sending your booking. Please call us on 0421 722 604.');
...
function showBookingError(msg) {
  ...
  if (!el) {
    ...
    el.id = 'bookingError';
    el.style.cssText = 'background:#FEE2E2;color:#DC2626;border:1px solid #FECACA;border-radius:8px;padding:0.85rem 1rem;margin-top:1rem;font-size:0.9rem;';
    const btn = document.querySelector('#bookingForm .btn-primary');
...
/* ---- CONTACT FORM — EmailJS only, no mailto, no email app ---- */
function submitContact() {
  ...
  emailjs.send(
    ...
    showFormStatus('error', 'Something went wrong. Please try again or call us on 0421 722 604.');
